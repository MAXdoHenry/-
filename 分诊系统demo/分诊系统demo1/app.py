...
def search_chat_messages(user_id: int, keyword: str) -> List[Dict[str, Any]]:
    """搜索当前用户可见的私聊和群聊消息；只查自己的好友会话和自己所在群。"""
    keyword = (keyword or "").strip()
    if not keyword:
        return []
    ...
    try:
        private_rows = conn.execute(
            ...
            SELECT
                'private' AS kind,
                CASE WHEN pm.sender_id = ? THEN pm.receiver_id ELSE pm.sender_id END AS target_id,
                u.name AS title,
                u.title AS subtitle,
                pm.content AS content,
                pm.send_time AS send_time
            ...
            WHERE pm.is_deleted = 0
              ...
              AND pm.content LIKE ?
            ORDER BY pm.send_time DESC, pm.id DESC
            LIMIT 30
            """,
            (user_id, user_id, user_id, user_id, like),
        ).fetchall()
        ...
        group_rows = conn.execute(
            ...
            SELECT
                ...
                gm.sender_name AS subtitle,
                gm.content AS content,
                gm.send_time AS send_time
            FROM group_messages gm
            JOIN groups g ON g.id = gm.group_id
            JOIN group_members mine ON mine.group_id = gm.group_id AND mine.user_id = ?
            WHERE gm.is_deleted = 0
              AND gm.content LIKE ?
            ORDER BY gm.send_time DESC, gm.id DESC
            LIMIT 30
            """,
            (user_id, like),
        ).fetchall()
    ...
    for row in private_rows:
        item = dict(row)
        item["url"] = url_for("private_chat", friend_id=item["target_id"])
        results.append(item)

    for row in group_rows:
        item = dict(row)
        item["url"] = url_for("group_chat", group_id=item["target_id"])
        results.append(item)

    results.sort(key=lambda item: item["send_time"] or "", reverse=True)
    return results[:40]
...
if app is not None:
    ...
    @app.route("/search")
    @login_required
    def global_search(current_user: Dict[str, Any]):
        """统一搜索：搜索自己的聊天记录，也可输入手机号或微信号查找医生加好友。"""
        query = request.args.get("q", "").strip()
        return render_template(
            "search.html",
            query=query,
            message_results=search_chat_messages(current_user["id"], query) if query else [],
            user_results=search_users(query, current_user["id"]) if query else [],
        )
